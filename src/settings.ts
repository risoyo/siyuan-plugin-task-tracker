...
function showHelpDialog(): void {
  ...
  <ul>
    <li><strong>表格</strong>：高密度编辑，可拖拽调整列宽。</li>
    <li><strong>清单</strong>：树形任务清单，适合逐项推进。</li>
    <li><strong>时间轴</strong>：按计划开始日期分组，未安排任务排在前面。</li>
    <li><strong>看板</strong>：按状态分列展示活跃任务。</li>
    <li><strong>日历</strong>：按计划开始日期展示任务，可展开未安排任务浮层。</li>
    <li><strong>已完成</strong>：按任务创建月份分组，可重新打开或删除任务。</li>
  </ul>

  <h2>六、完成、删除与归档</h2>
  <p>顶层任务标记为已完成后，会自动移动到 <code>事项库/已完成/YYYY-MM</code>，月份按任务创建时间计算。子任务单独完成时会保持在父任务下。</p>
